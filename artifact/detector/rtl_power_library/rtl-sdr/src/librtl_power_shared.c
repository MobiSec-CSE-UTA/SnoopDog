...
double get_time_now(){
	...
	return _x.tv_sec + (0.000001 * _x.tv_usec);
}
...
double poisson(int i, int length)
{
	double a, N1, w;
	a = 2.0;
	N1 = (double)(length-1);
...
double hann_pure(int i, int length)
{
	double a, N1, w;
	a = 2.0;
	N1 = (double)(length-1);
...
int get_dbm_in_double_array(struct tuning_state *ts,double* result_header,double* results)
{
	...
	*(result_header+0) = (double)(ts->freq - bw2);
	*(result_header+1) = (double)(ts->freq + bw2);
	*(result_header+2) = (double)ts->rate / (double)(len*ds);
	*(result_header+3) = (double)(ts->samples);
	int size_of_darray = 0;
	...
	*(results+darray_indexer) = dbm;
	...
	ts->samples = 0;
	*(result_header+4) = (double)(size_of_darray);
	return size_of_darray;
}
...
int init_rtl_sdr( const char* a_freq_optarg, int a_gain, double a_interval,double a_exit_time)
{
	...
	// }
	crop = 0.2;
	frequency_range(a_freq_optarg, crop);
	// fprintf(stderr,"time for target freq change : %f\n",get_time_now()- old_time);
	// if (interval < 1) {
		// interval = 1;}

	// fprintf(stderr, "Reporting every %f seconds\n", interval);

	if (!dev_given && dev==NULL) {
...
int read_rtl_sdr_samples(CALLBACK cb){
	...
	exit_time = (double)(input_exit_time);
	/* actually do stuff */
	rtlsdr_set_sample_rate(dev, (uint32_t)tunes[0].rate);
	sine_table(tunes[0].bin_e);
	next_tick = get_time_now() + interval;
	if (exit_time) {
		exit_time = get_time_now() + exit_time;
	}
	fft_buf = malloc(tunes[0].buf_len * sizeof(int16_t));
	len_buff_ = tunes[0].buf_len * sizeof(int16_t);
	...
	while (!do_exit) {
		...
		double db_array[204800] = {0.};
		double db_array_header[10240] = {0.};
		int array_indexer = 0;
		int size_of_darray = 0;
		int total_size_of_array = 0;
		for (i=0; i<tune_count; i++) {
			size_of_darray = get_dbm_in_double_array(&tunes[i],&db_array_header[i*5],&db_array[array_indexer]);
			array_indexer += size_of_darray;
		}
		cb( db_array_header, db_array, 5*tune_count, array_indexer);
		while (get_time_now() >= next_tick) {
			next_tick += interval;}
		if (single) {
			do_exit = 1;}
		if (get_time_now() >= exit_time) {
			do_exit = 1;}
	}
	free(fft_buf);
	free(window_coefs);
